...
const PLANS = {
  personal: [
    ...
    },
    {
      ...
      price: 'R$ 29,90',
      period: '/mês',
      features: ['Análise Completa (Gemini 3 Pro)', 'Geração em 4K', 'Editor Mágico (Nano Banana)', 'Colorimetria Avançada'],
    ...
    <div className="min-h-screen relative overflow-hidden bg-slate-900 flex flex-col items-center justify-center">
      ...
      <div className="relative z-20 px-8 text-center max-w-md w-full animate-in slide-in-from-bottom duration-700">
        ...
        <p className="text-slate-300 text-lg mb-8 leading-relaxed">
          Nossa IA (Gemini 3 Pro) processou seus traços. Desbloqueie o resultado.
        </p>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <div className="p-6 bg-white shadow-sm z-10">
        <div className="flex items-center mb-4">
          <button onClick={onBack} className="mr-4"><ArrowRight className="w-6 h-6 rotate-180" /></button>
          <h2 className="text-2xl font-serif text-slate-900">Seu Guarda-Roupa</h2>
        </div>
        <p className="text-slate-500 text-sm">
          Looks gerados com Gemini 3 Pro Image.
        </p>
      </div>
      ...
      <div className="flex-1 p-6 grid grid-cols-2 gap-4 overflow-y-auto">
        {looks.length === 0 ? (
          <div className="col-span-2 flex flex-col items-center justify-center text-slate-400 py-10">
            ...
            <p className="text-sm">Seu guarda-roupa está vazio.</p>
          </div>
        ) : (
          looks.map((look) => (
            <div 
              ...
              className="relative aspect-[3/4] bg-slate-200 rounded-2xl overflow-hidden shadow-md cursor-pointer group"
            >
              <img 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-3">
                <span className="text-white text-xs font-bold uppercase tracking-wider mb-1">{look.titulo}</span>
              </div>
            </div>
          ))
        )}
...
const VizuhalizandoApp = () => {
  ...
  // Generate Look with Gemini 3 Pro Image
  ...
  // Edit Look with Gemini 2.5 Flash Image (Nano Banana)
      ...
      <div className="min-h-screen bg-white flex flex-col p-6">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-slate-500 mb-8 max-w-xs">
            Envie sua foto para que o <strong>Gemini 3 Pro</strong> analise seu rosto.
          </p>
      ...
      <div className="min-h-screen bg-white flex flex-col">
        <div className="p-6 border-b border-slate-100 flex items-center bg-slate-50">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Powered by Gemini 3 Pro Image</p>
          </div>
        </div>
        
        <div className="p-6 space-y-6">
          
          {/* Resolution Selector Feature */}
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            <label className="text-xs font-bold uppercase text-slate-500 mb-2 block">Resolução de Imagem</label>
            <div className="flex gap-2">
              {(['1K', '2K', '4K'] as ImageResolution[]).map((res) => (
                <button
                  key={res}
                  onClick={() => setSelectedResolution(res)}
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${selectedResolution === res ? 'bg-violet-600 text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-200'}`}
                >
                  {res}
                </button>
              ))}
          ...
          <div className="flex items-center gap-3 p-3 bg-violet-50 rounded-xl border border-violet-100">
             <input 
               ...
               onChange={(e) => setCreateEnvironment(e.target.checked)}
          ...
          <div className="grid grid-cols-1 gap-4">
            {LOOK_OBJECTIVES.map((obj) => (
              <button
                ...
                className="relative overflow-hidden flex items-center p-4 rounded-xl border border-slate-200 hover:border-violet-500 transition-all text-left group h-24"
              >
                <div className="relative z-10 flex items-center w-full">
                  <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mr-4 shadow-sm group-hover:scale-110 transition-transform">
                    <obj.icon className="w-6 h-6 text-slate-800" />
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-900 text-lg">{obj.label}</h4>
                  </div>
